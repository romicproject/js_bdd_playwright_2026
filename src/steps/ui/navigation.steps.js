...
When(
  "I navigate to the {string} page from home",
  async ({ homePage }, navItem) => {
    await homePage.clickTopNavigation(navItem);
  },
);

Then(
  "I should see the Contact Us heading and URL",
  async ({ contactUsPage }) => {
...
When(
  "I search for UI product {string}",
  async ({ productsPage }, searchTerm) => {
    await productsPage.searchProduct(searchTerm);
  },
);

Then(
  "I should see searched products related to {string}",
  async ({ productsPage }, searchTerm) => {
