...
When(
  "I register a new user with unique credentials",
  async ({ homePage, loginPage, registerPage, uiContext }) => {
    await createAccountWithUniqueUser({
      homePage,
      loginPage,
      registerPage,
      uiContext,
...
Then(
  "I should see the account created confirmation",
  async ({ registerPage }) => {
...
Then(
  "I should see the user logged in successfully",
  async ({ registerPage, uiContext }) => {
...
Given(
  "an account exists for a unique user",
  async ({ homePage, loginPage, registerPage, uiContext }) => {
    await ensureAccountCreatedAndLoggedIn({
      homePage,
      loginPage,
      registerPage,
      uiContext,
...
When(
  "I try to register again with the same email",
  async ({ homePage, loginPage, uiContext }) => {
...
Then(
  "I should see an email already exists error message",
  async ({ loginPage, uiContext }) => {
